# Porcupine Wake Word Engine + Rhino Speech-To-Intent Engine Demo for Arduino (Korean language)
...
3. Search for `Picovoice_KO`, then click `INSTALL`.
...
1. Open `File`->`Examples`->`Picovoice_KO`->`MicrocontrollerVoiceAssistantExample`.
...
1. Compile and upload the `Porcupine_KO/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
